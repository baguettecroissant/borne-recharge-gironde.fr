#!/usr/bin/env node
/**
 * Verify content uniqueness across all local pages.
...
for (const category of CATEGORIES) {
  ...
  for (const c of communes) {
    const combo = [
      ...
      c.distanceBordeaux <= 5 ? 'core' : c.distanceBordeaux <= 15 ? 'inner' : c.distanceBordeaux <= 35 ? 'mid' : 'outer',
...
for (const c of communes) {
  ...
  distanceSet.add(c.distanceBordeaux);
