...
async function fetchCities() {
  const url = 'https://geo.api.gouv.fr/departements/33/communes?fields=nom,code,population,codesPostaux,centre';
  
  console.log('Fetching Gironde (33) communes from Geo API...');
  try {
    ...
    // Process and filter communes in Gironde (33) with population >= 2000
    const processed = data
      .map(c => {
        let lat = 44.837789; // Bordeaux lat
        let lng = -0.57918;  // Bordeaux lng
